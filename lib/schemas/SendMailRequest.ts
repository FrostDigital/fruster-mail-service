...
module.exports = {
	...
	description: "Request for sending mail. Can be used in two ways: mail contents defined in request or with template. If contents is in request; `to`, `subject` and `message` are required. If template is used; `to` and `templateId` are required. ",
	properties: {
		to: {
			type: ["string", "array"],
			items: {
		...
		subject: {
			type: "string",
			description: "Required if `templateId` is not used! The subject of the mail being sent",
			default: "Hello from anywhere else"
		},
		message: {
			type: "string",
			description: "Required if `templateId` is not used! The mail body being sent",
			default: "&lt;h1&gt;Hello&lt;/h1&gt; This is the body of the mail!"
		},
		templateId: {
			type: "string",
			description: "Required if `message` is not used! An optional id of a sendgrid template to use. If this is used, the message field is ignored.",
